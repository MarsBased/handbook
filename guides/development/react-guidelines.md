...
- Use a library (with hooks) for global state management, for example [zustand](https://github.com/pmndrs/zustand)
...
There is no `pages/` folder: the Pages Router is legacy. Only route files (`page.tsx`, `layout.tsx`, `route.ts`, etc.) live in `app/`; everything reusable goes to `components/`.
...
- You probably don't need redux. Hooks plus zustand cover almost every case. If redux is unavoidable, use Redux Toolkit and its hooks API
...
- Only route files live in `app/`. Everything reusable goes to `components/`
...
- Styling
  ...
  - [styled-components](https://styled-components.com/) (in maintenance mode, don't start new projects with it)
- Components
